...
def cgb_input_main(inputfile_name):
    ...
    log_file = collections.OrderedDict()
    ...
    log_file['cgb_parameters'] = cgb_pipeline_parameters
    ...
    output['cgb_parameters'] = cgb_pipeline_parameters
    ...
    for item in motifs:
        ...
        for prot_rec in records['IPGReport']['ProteinList']:
            ...
            if item['protein_accession'] == prot_rec.attributes['accver']:
    ...
    for accession in TF_accessions:
        ...
        blast_hits = blast_search(accession, blast_db,
                                             evalue, nhits,
    ...
    num_log['num orthologs before processing'] = num_orthologs
    print num_orthologs, 'hits before processing'
    ...
    for ortholog in orthologs:
        ...
        if cds_accession:
            ...
            if not target_range:
                ...
                log_file[ortholog] = 'contig_accessions returned None for target range'
            ...
            log_file[ortholog] = 'genome_retrieval returned None; '\
                                 'issue concerning IPG report most likely'
    ...
    num_log['num orthologs removed in genome retrieval'] = gen_ret_cnt

    num_log['num orthologs removed in contig accessions'] = contig_cnt

    num_log['num orthologs removed in genome retrieval or contig accessions' \
            ' (unclear)'] = other_cnt

    num_log['num orthologs remaining post-contig_accessions, pre-filters'] = num_orthologs

    with open(orth_dir_prefilt, 'w') as f:

        json.dump(orthologs, f, indent=2, separators=(',', ': '))

    cnt_lost_pre_filt = other_cnt + contig_cnt + gen_ret_cnt

    num_log['total num orthologs removed before filters'] = cnt_lost_pre_filt

    print '\nTotal number of valid orthologs detected: ' + str(num_orthologs)
    ...
    if selected_taxon:
        ...
        num_orthologs = len(filtered_orthologs)

        cnt_lost_post_TaxFilt = len(orthologs) - num_orthologs

        num_log['orthologs lost due to taxon filtering'] = cnt_lost_post_TaxFilt

        num_log['orthologs remaining after taxon filtering'] = num_orthologs
    ...
    if maxID:
        ...
        num_log['num orthologs remaining pre-maxID filter'] = num_orthologs

        filtered_list = maxID_filt(maxID, filtered_orthologs, up_region,
                                   dw_region, sleepy)

        num_rem_maxID = num_orthologs - len(filtered_list)

        num_log['num orthologs removed by maxID filter'] = num_rem_maxID

        num_orthologs = len(filtered_list)

        num_log['num orthologs remaining post-maxID filter'] = num_orthologs
        ...
        for ortholog in filtered_orthologs:
            ...
            if ortholog in filtered_list:
                ...
                log_maxID[ortholog] = '%ID too high'
...
# =====================================
# inputfile_name = '../Cprotein_test.json'
# =====================================
# ### ENTER YOUR INPUT FILE PATH HERE ###
